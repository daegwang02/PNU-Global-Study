...
class CareerScraper:
    def __init__(self):
        ...
        self.keywords = {
            ...
            "경제": [
                ...
                "미래에셋", "삼성증권", "한국투자", "한국거래소", "한국은행",
                # 직무
                "PB", "WM", "자산관리", "신용평가", "기획", "여신",
                "금융일반", "영업", "주식", "외환", "리서치", "리스크관리",
                "경영기획", "전략기획", "IR", "영업관리",
                # 자격증
                "CPA", "AICPA", "CFA", "FRM", "CFP",
                # 기타
    ...
    def parse_rss_feeds(self):
        ...
        for feed_url in rss_sources["jobs"]:
            try:
                print(f"  > [채용] {feed_url} 확인 중...") # 👈 [추가] 로그
                is_pnu_feed = "pusan.ac.kr" in feed_url # 👈 [추가] 부산대 피드인지 확인
                ...
                for entry in feed.entries[:20]:
                    ...
                    # 👈 [추가] 부산대 공지는 키워드가 없으면 '정치외교'로 자동 분류
                    if not category:  # 관련 없음
                        if is_pnu_feed:
                            category = "정치외교" # 기본 카테고리 할당
                        else:
                            continue # 다른 사이트는 스킵
                    ...
                    job = {
                        ...
                        "deadline": self.extract_deadline(entry),
                        "url": entry.get('link', ''),
                        "tags": self.extract_tags(title + " " + entry.get('link', '')), # 👈 [수정] 태그 분석 시 URL도 포함
                        "new": True
                    }
        ...
        for feed_url in rss_sources["contests"]:
            try:
                print(f"  > [공모전] {feed_url} 확인 중...") # 👈 [추가] 로그
                is_pnu_feed = "pusan.ac.kr" in feed_url # 👈 [추가] 부산대 피드인지 확인
    ...
    def extract_deadline(self, entry) -> str:
        """마감일 추출"""
        # (개선된 마감일 추출 로직이 필요하면 여기에 추가 - 지금은 30일로 유지)
        future_date = datetime.now() + timedelta(days=30)
        return future_date.strftime('%Y-%m-%d')
    ...
    def extract_tags(self, text: str) -> List[str]:
        ...
        # 👈 [추가] URL이나 제목에 '부산'이 들어가면 "부산대" 태그 강제 추가
        if "pusan.ac.kr" in text_lower or "부산대학교" in text_lower:
            tags.append("부산대")
            
        for keyword in tag_keywords:
            if keyword in text_lower:
                if keyword not in tags: # 👈 [추가] 중복 방지
                    tags.append(keyword)
        return tags[:5]
...
def main():
    print("=" * 50)
    print("🤖 자동 데이터 수집 시작 (v_PNU)") # 👈 [수정] 버전 표시
    print("=" * 50)
